...
namespace Wintools {
    internal sealed partial class MainWindow {
        ...
        private void AdaptLayout(){
            ...
            string[] labels={"Каталог действий","История и откат","Подборки","Настройки","План изменений","Службы сейчас","Диагностика ПК","Сверить настройки","Ускорение Windows","Приложения","Проверка сети","Обслуживание","Автозагрузка"};
            for(int i=0;i<nav.Length;i++){var button=Get<Button>(nav[i]);button.Content=labels[i];button.ToolTip=labels[i];button.FontSize=compact?12:14;button.Padding=compact?new Thickness(8,5,8,5):new Thickness(14,12,14,12);button.MinHeight=compact?32:40;button.Margin=compact?new Thickness(0,1,0,1):new Thickness(0,3,0,3);}
            var dock=(DockPanel)sidebar.Child;dock.Children[0].Visibility=compact?Visibility.Collapsed:Visibility.Visible;dock.Children[1].Visibility=compact||Window.ActualHeight<1140?Visibility.Collapsed:Visibility.Visible;
