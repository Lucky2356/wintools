...
namespace Wintools {
    internal sealed class ServiceSnapshot {
        ...
        public bool Delayed, CanStop, CanPause;
    }
    ...
    internal static class ServiceActions {
        ...
        internal static bool Manageable(ServiceSnapshot service){return service!=null&&service.Kind!=null&&service.Kind.IndexOf("Process",StringComparison.OrdinalIgnoreCase)>=0&&new[]{"Running","Stopped","Paused"}.Contains(service.State)&&new[]{"Auto","Manual","Disabled"}.Contains(service.Mode);}
        ...
        private static void Start(string name){var current=Inspect(name);if(current.State=="Running")return;if(current.State!="Stopped")throw new IOException("Состояние службы изменилось. Для приостановленной службы используйте «Продолжить».");Invoke(name,"StartService",null);WaitState(name,"Running");}
        private static void Pause(string name){var current=Inspect(name);if(current.State=="Paused")return;if(current.State!="Running"||!current.CanPause)throw new IOException("Служба сейчас не поддерживает приостановку.");Invoke(name,"PauseService",null);WaitState(name,"Paused");}
        private static void Resume(string name){var current=Inspect(name);if(current.State=="Running")return;if(current.State!="Paused"||!current.CanPause)throw new IOException("Служба сейчас не поддерживает продолжение работы.");Invoke(name,"ResumeService",null);WaitState(name,"Running");}
        ...
        private static void Restore(ServiceSnapshot before){
            if(before==null||!ValidName(before.Name)||!new[]{"Auto","Manual","Disabled"}.Contains(before.Mode)||!new[]{"Running","Stopped","Paused"}.Contains(before.State))throw new IOException("Некорректное исходное состояние службы.");
            if(before.State=="Stopped")Stop(before.Name);
            bool active=before.State!="Stopped";
            Mode(before.Name,before.Mode=="Disabled"&&active?"Manual":before.Mode,before.Delayed);
            if(before.State=="Running"){if(Inspect(before.Name).State=="Paused")Resume(before.Name);else Start(before.Name);}
        ...
        private static void Validate(string name,string action,string restoreId){
            if(!ValidName(name)||!new[]{"start","stop","restart","pause","resume","auto","delayed","manual","disabled","restore"}.Contains(action))throw new ArgumentException("Недопустимое действие службы.");
        ...
        internal static int Worker(string[] args){
            ...
            using(var log=new StreamWriter(new FileStream(Path.Combine(runtime,id+".service.log"),FileMode.CreateNew,FileAccess.Write,FileShare.Read),new UTF8Encoding(false))){
                ...
                try{gate=new FileStream(lockPath,FileMode.CreateNew,FileAccess.Write,FileShare.None);var before=Inspect(name);if(!Manageable(before))throw new IOException("Дождитесь стабильного состояния службы. Драйверы этим инструментом не изменяются.");
                    ...
                    if(action=="pause"&&(before.State!="Running"||!before.CanPause))throw new IOException("Служба не поддерживает приостановку в текущем состоянии.");
                    if(action=="resume"&&(before.State!="Paused"||!before.CanPause))throw new IOException("Служба не поддерживает продолжение в текущем состоянии.");
                    if(action=="start"&&before.State=="Paused")throw new IOException("Служба приостановлена. Используйте «Продолжить».");
                    ...
                    if(action=="start")Start(name);else if(action=="pause")Pause(name);else if(action=="resume")Resume(name);else if(action=="stop")Stop(name);else if(action=="restart"){Stop(name);Start(name);}else if(action=="restore")Restore(original.Before);else Mode(name,action=="manual"?"Manual":action=="disabled"?"Disabled":"Auto",action=="delayed");
