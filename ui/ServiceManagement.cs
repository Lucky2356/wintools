...
namespace Wintools {
    internal sealed partial class MainWindow {
        ...
        private Button serviceStart,serviceStop,serviceRestart,serviceModeApply,serviceRestore,servicePause;
        ...
        private void InitializeServiceManagement(Grid footer){
            ...
            serviceStart=ServiceControlButton("Запустить",null);serviceStart.Click+=async(s,e)=>await ChangeSelectedService(selectedServiceSnapshot!=null&&selectedServiceSnapshot.State=="Paused"?"resume":"start");servicePause=ServiceControlButton("Приостановить","pause");serviceStop=ServiceControlButton("Остановить","stop");serviceRestart=ServiceControlButton("Перезапустить","restart");
        ...
        private void RefreshServiceControls(){
            ...
            serviceControls.Visibility=serviceList.SelectedItem==null?Visibility.Collapsed:Visibility.Visible;serviceStart.Content=current!=null&&current.State=="Paused"?"Продолжить":"Запустить";serviceStart.IsEnabled=ready&&(current.State=="Stopped"&&current.Mode!="Disabled"||current.State=="Paused"&&current.CanPause);servicePause.Visibility=current!=null&&current.CanPause&&current.State=="Running"?Visibility.Visible:Visibility.Collapsed;servicePause.IsEnabled=ready&&current.State=="Running"&&current.CanPause;serviceStop.IsEnabled=ready&&(current.State=="Running"||current.State=="Paused")&&current.CanStop;serviceRestart.IsEnabled=serviceStop.IsEnabled&&current.Mode!="Disabled";serviceModeApply.IsEnabled=serviceStartMode.IsEnabled=ready;serviceRestore.IsEnabled=ready&&selectedServiceRestore!=null;
        }
        ...
        private async Task ChangeSelectedService(string action){
            ...
            string effect=action=="pause"?"Служба останется запущенной, но приостановит обработку работы в соответствии со своей реализацией. Связанные функции могут стать недоступны. Это не отключение автозапуска и не гарантированное освобождение памяти.":action=="resume"?"Приостановленная служба продолжит работу. Тип запуска не изменится.":action=="start"?"Windows запустит службу и при необходимости её зависимости.":action=="stop"?"Служба остановится сейчас. Связанные функции и подключения могут перестать работать. Зависимые службы автоматически не останавливаются.":action=="restart"?"Служба остановится и запустится снова. Её подключения будут прерваны.":action=="restore"?"Вернём тип запуска и состояние выбранной службы перед последним сохранённым действием. Более поздние изменения других программ могут быть перезаписаны. Зависимости отдельно не восстанавливаются.":"Тип запуска станет: "+(action=="auto"?"автоматический":action=="delayed"?"автоматический с задержкой":action=="manual"?"вручную / по запросу":"отключённый")+". Работающая служба сама по себе не остановится.";
        ...
        private static string ServiceActionTitle(string action){return action=="pause"?"Приостановка":action=="resume"?"Продолжение":action=="start"?"Запуск":action=="stop"?"Остановка":action=="restart"?"Перезапуск":action=="restore"?"Восстановление":action=="auto"?"Автоматический запуск":action=="delayed"?"Отложенный запуск":action=="manual"?"Ручной запуск":"Отключение запуска";}
