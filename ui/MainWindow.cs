...
namespace Wintools {
    ...
    internal sealed class HistoryRow {
        ...
        internal bool PowerChange,IntegrityCheck,StartupChange,ProcessChange;
        internal DateTime TimeUtc;
        public string Detail {get{return TimeUtc==DateTime.MinValue?Run:TimeUtc.ToLocalTime().ToString("g")+" · "+(ProcessChange?"Процесс":StartupChange?"Автозагрузка":IntegrityCheck?"Обслуживание Windows":PowerChange?"Питание":ServiceName==null?"Запуск":ServiceName);}}
    }
    internal sealed partial class MainWindow {
        ...
        private int page;
        private readonly string[] pages={"CataloguePage","HistoryPage","CollectionsPage","SettingsPage","PlanPage","ServicesPage","HealthPage","VerificationPage","OptimizationPage","ApplicationsPage","NetworkPage","IntegrityPage","StartupPage","ProcessesPage"};
        private readonly string[] nav={"NavCatalogue","NavHistory","NavCollections","NavSettings","NavPlan","NavServices","Diagnose","Verify","NavOptimize","NavApplications","NavNetwork","NavIntegrity","NavStartup","NavProcesses"};
        ...
        internal MainWindow(bool smokeMode) {
            ...
            InitializeCollections();InitializeHealth();InitializeApplications();InitializeNetworkDiagnostics();InitializeIntegrity();InitializeStartup();InitializeProcesses();
            ...
            ClickAsync("HistoryRevert",async()=>{var row=Get<ListBox>("History").SelectedItem as HistoryRow;if(row!=null&&row.ProcessChange){await RestoreProcessHistory(row.Run);return;}if(row!=null&&row.StartupChange){await RestoreStartupHistory(row.Run);return;}if(row!=null&&row.PowerChange){await RestorePowerHistory(row.Run);return;}if(row!=null&&row.ServiceName!=null){await RestoreServiceHistory(row);return;}if(row!=null&&row.CanRevert&&await Confirm("Откатить запуск "+row.Run+"?\n\nСначала откатывайте более новые изменения."))await Run("revert",null,row.Run,false);});
        ...
        private void ShowPage(int index) {
            ...
            Text("PageTitle",new[]{"Windows под ваши задачи","История изменений","С чего начать","Настройки приложения","Ваш план изменений","Службы сейчас","Состояние вашего ПК","Сверка настроек","Ускорение Windows","Установленные приложения","Проверка соединения","Целостность Windows","Автозагрузка программ","Работающие процессы"}[index]);
        ...
        private void RefreshEnabled() {
            ...
            RefreshServiceControls();RefreshPowerEnabled();RefreshStartupEnabled();RefreshProcessEnabled();RefreshIntegrityEnabled();var historySelection=Get<ListBox>("History").SelectedItem as HistoryRow;Visible("HistoryReport",historySelection!=null&&historySelection.IntegrityCheck);Enabled("HistoryReport",!busy&&historySelection!=null&&historySelection.IntegrityCheck);if(applicationList!=null)ApplicationSelection();RefreshPlanEnabled();foreach(var button in collectionPlanButtons)button.IsEnabled=!busy;Enabled("ProfileImport",!busy);Enabled("ProfileExport",!busy&&preferences.Plan.Count>0);
        ...
        private void ReadHistory() {
            try{var rows=HistoryRows(Path.Combine(Program.Data,"state","applied.dat"),catalogue).Concat(ServiceHistoryRows()).Concat(PowerHistoryRows()).Concat(IntegrityHistoryRows()).Concat(StartupHistoryRows()).Concat(ProcessHistoryRows()).OrderByDescending(r=>r.TimeUtc).ToArray();Get<ListBox>("History").ItemsSource=rows;Text("HistoryStatus",rows.Length==0?"Изменений пока нет. После выполнения действия здесь появится запись.":"Запусков: "+rows.Length+". Сначала откатывайте самые новые изменения.");}
        ...
        private async Task Smoke() {
            ...
            planCalls=0;planAction=(item,dry,progress)=>Task.FromResult(new EngineResult{Code=++planCalls==1?0:4,Output="fixture result"});await RunPlan(false);Assert(planCalls==2&&preferences.Plan.Count==1&&Preferences.Load().Plan.Count==1,"Plan did not stop and retain failed item: calls="+planCalls+"; memory="+preferences.Plan.Count+"; saved="+Preferences.Load().Plan.Count+"; output="+Get<TextBox>("Output").Text);
            ...
            await Task.Delay(250);Get<ComboBox>("Theme").SelectedIndex=2;Assert(Preferences.Load().Theme=="dark","Theme persistence: "+Get<TextBlock>("Status").Text);
            ...
            await HealthSmoke();await WorkspaceSmoke();await MonitorSmoke();await ApplicationSmoke();await NetworkSmoke();await ServiceManagementSmoke();await ServiceWatchingSmoke();await PowerSmoke();await PlanPersistenceSmoke();await IntegritySmoke();await StartupSmoke();await WindowsRepairSmoke();await ProcessSmoke();await HardwareSmoke();ShowPage(0);Window.Width=Window.MinWidth;Window.Height=Window.MinHeight;ExpandOutput(true);Window.UpdateLayout();await Task.Delay(100);Capture("portable-ui-compact.png");Assert(IsVisibleInWindow("Apply")&&IsVisibleInWindow("Star")&&IsVisibleInWindow("ActionTitle")&&IsVisibleInWindow("Metadata")&&IsVisibleInWindow("Verify"),"Compact clipping: Apply="+IsVisibleInWindow("Apply")+" Star="+IsVisibleInWindow("Star")+" Title="+IsVisibleInWindow("ActionTitle")+" Metadata="+IsVisibleInWindow("Metadata")+" Verify="+IsVisibleInWindow("Verify"));Capture("portable-ui-compact.png");
