...
namespace Wintools {
    ...
    internal sealed partial class MainWindow {
        ...
        private async Task Smoke() {
            ...
            await HealthSmoke();await WorkspaceSmoke();await MonitorSmoke();await ApplicationSmoke();await NetworkSmoke();PrepareHistorySmoke();await ServiceManagementSmoke();await ServiceWatchingSmoke();await PowerSmoke();await PlanPersistenceSmoke();await IntegritySmoke();await StartupSmoke();await WindowsRepairSmoke();await ProcessSmoke();await HardwareSmoke();await StoreSmoke();await GpuSmoke();await MeasurementSmoke();await ServiceDependencySmoke();await CollectionAssistantSmoke();HistoryIsolationSmoke();ShowPage(0);Window.Width=Window.MinWidth;Window.Height=Window.MinHeight;ExpandOutput(true);Window.UpdateLayout();await Task.Delay(100);Capture("portable-ui-compact.png");Assert(IsVisibleInWindow("Apply")&&IsVisibleInWindow("Star")&&IsVisibleInWindow("ActionTitle")&&IsVisibleInWindow("Metadata")&&IsVisibleInWindow("Verify"),"Compact clipping: Apply="+IsVisibleInWindow("Apply")+" Star="+IsVisibleInWindow("Star")+" Title="+IsVisibleInWindow("ActionTitle")+" Metadata="+IsVisibleInWindow("Metadata")+" Verify="+IsVisibleInWindow("Verify"));Capture("portable-ui-compact.png");
