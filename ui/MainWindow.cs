...
namespace Wintools {
    ...
    internal sealed class HistoryRow {
        ...
        public string Detail {get{return TimeUtc==DateTime.MinValue?Run:TimeUtc.ToLocalTime().ToString("g")+" · "+(StartupChange?"Автозагрузка":IntegrityCheck?"Проверка Windows":PowerChange?"Питание":ServiceName==null?"Запуск":ServiceName);}}
    }
    internal sealed partial class MainWindow {
        ...
        private async Task Smoke() {
            ...
            await HealthSmoke();await WorkspaceSmoke();await MonitorSmoke();await ApplicationSmoke();await NetworkSmoke();await ServiceManagementSmoke();await ServiceWatchingSmoke();await PowerSmoke();await PlanPersistenceSmoke();await IntegritySmoke();await StartupSmoke();ShowPage(0);Window.Width=Window.MinWidth;Window.Height=Window.MinHeight;ExpandOutput(true);Window.UpdateLayout();await Task.Delay(100);Capture("portable-ui-compact.png");Assert(IsVisibleInWindow("Apply")&&IsVisibleInWindow("Star")&&IsVisibleInWindow("ActionTitle")&&IsVisibleInWindow("Metadata")&&IsVisibleInWindow("Verify"),"Compact clipping: Apply="+IsVisibleInWindow("Apply")+" Star="+IsVisibleInWindow("Star")+" Title="+IsVisibleInWindow("ActionTitle")+" Metadata="+IsVisibleInWindow("Metadata")+" Verify="+IsVisibleInWindow("Verify"));Capture("portable-ui-compact.png");
