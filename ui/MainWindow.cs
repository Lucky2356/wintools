...
namespace Wintools {
    ...
    internal sealed class HistoryRow {
        ...
        internal bool PowerChange,IntegrityCheck,StartupChange,ProcessChange,StoreChange;
        internal DateTime TimeUtc;
        public string Detail {get{return TimeUtc==DateTime.MinValue?Run:TimeUtc.ToLocalTime().ToString("g")+" · "+(StoreChange?"Приложение":ProcessChange?"Процесс":StartupChange?"Автозагрузка":IntegrityCheck?"Обслуживание Windows":PowerChange?"Питание":ServiceName==null?"Запуск":ServiceName);}}
    }
    internal sealed partial class MainWindow {
        ...
        private void ReadHistory() {
            try{var rows=HistoryRows(Path.Combine(Program.Data,"state","applied.dat"),catalogue).Concat(ServiceHistoryRows()).Concat(PowerHistoryRows()).Concat(IntegrityHistoryRows()).Concat(StartupHistoryRows()).Concat(ProcessHistoryRows()).Concat(StoreHistoryRows()).OrderByDescending(r=>r.TimeUtc).ToArray();Get<ListBox>("History").ItemsSource=rows;Text("HistoryStatus",rows.Length==0?"Изменений пока нет. После выполнения действия здесь появится запись.":"Запусков: "+rows.Length+". Сначала откатывайте самые новые изменения.");}
        ...
        private async Task Smoke() {
            ...
            await HealthSmoke();await WorkspaceSmoke();await MonitorSmoke();await ApplicationSmoke();await NetworkSmoke();await ServiceManagementSmoke();await ServiceWatchingSmoke();await PowerSmoke();await PlanPersistenceSmoke();await IntegritySmoke();await StartupSmoke();await WindowsRepairSmoke();await ProcessSmoke();await HardwareSmoke();await StoreSmoke();await GpuSmoke();await MeasurementSmoke();await ServiceDependencySmoke();ShowPage(0);Window.Width=Window.MinWidth;Window.Height=Window.MinHeight;ExpandOutput(true);Window.UpdateLayout();await Task.Delay(100);Capture("portable-ui-compact.png");Assert(IsVisibleInWindow("Apply")&&IsVisibleInWindow("Star")&&IsVisibleInWindow("ActionTitle")&&IsVisibleInWindow("Metadata")&&IsVisibleInWindow("Verify"),"Compact clipping: Apply="+IsVisibleInWindow("Apply")+" Star="+IsVisibleInWindow("Star")+" Title="+IsVisibleInWindow("ActionTitle")+" Metadata="+IsVisibleInWindow("Metadata")+" Verify="+IsVisibleInWindow("Verify"));Capture("portable-ui-compact.png");
