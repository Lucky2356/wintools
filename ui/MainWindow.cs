...
namespace Wintools {
    ...
    internal sealed class HistoryRow {
        ...
        internal bool PowerChange,IntegrityCheck,StartupChange;
        internal DateTime TimeUtc;
        public string Detail {get{return TimeUtc==DateTime.MinValue?Run:TimeUtc.ToLocalTime().ToString("g")+" · "+(StartupChange?"Автозагрузка":IntegrityCheck?"Обслуживание Windows":PowerChange?"Питание":ServiceName==null?"Запуск":ServiceName);}}
    }
    internal sealed partial class MainWindow {
        ...
        private int page;
        private readonly string[] pages={"CataloguePage","HistoryPage","CollectionsPage","SettingsPage","PlanPage","ServicesPage","HealthPage","VerificationPage","OptimizationPage","ApplicationsPage","NetworkPage","IntegrityPage","StartupPage"};
        private readonly string[] nav={"NavCatalogue","NavHistory","NavCollections","NavSettings","NavPlan","NavServices","Diagnose","Verify","NavOptimize","NavApplications","NavNetwork","NavIntegrity","NavStartup"};
        ...
        internal MainWindow(bool smokeMode) {
            ...
            InitializeCollections();InitializeHealth();InitializeApplications();InitializeNetworkDiagnostics();InitializeIntegrity();InitializeStartup();
        ...
        private void ShowPage(int index) {
            ...
            Text("PageTitle",new[]{"Windows под ваши задачи","История изменений","С чего начать","Настройки приложения","Ваш план изменений","Службы сейчас","Состояние вашего ПК","Сверка настроек","Ускорение Windows","Установленные приложения","Проверка соединения","Целостность Windows","Автозагрузка программ"}[index]);
            Text("PageEyebrow",new[]{"КАТАЛОГ ДЕЙСТВИЙ","ЖУРНАЛ ЭТОГО КОМПЬЮТЕРА","ПОДБОРКИ","ВАШИ ПРЕДПОЧТЕНИЯ","ПОДГОТОВКА И ВЫПОЛНЕНИЕ","РАБОТА И АВТОЗАПУСК","ПОНЯТНАЯ ДИАГНОСТИКА","ПРОВЕРКА БЕЗ ИЗМЕНЕНИЙ","ПРАКТИЧЕСКИЕ ШАГИ","ПРОГРАММЫ НА КОМПЬЮТЕРЕ","ДИАГНОСТИКА СЕТИ","ОБСЛУЖИВАНИЕ","ЗАПУСК ПРИ ВХОДЕ"}[index]);
            Text("PageHint",new[]{"Выберите раздел или найдите нужное действие.","Исходные состояния и откат сохранённых запусков.","Три подборки с настройкой под ваши задачи.","Автообновление, защита и данные приложения.","Соберите действия, проверьте и выполните по порядку.","Снимок установленных служб Windows.","Показатели и подсказки вместо технического лога.","Сохранились ли применённые настройки?","Выберите улучшение под свою задачу.","Поиск, размер и удаление настольных программ.","Задержка, ответы сервера и стабильность соединения.","Проверка и восстановление компонентов и системных файлов.","Выберите, какие программы нужны сразу после входа."}[index]);
        ...
        private void RefreshEnabled() {
            ...
            RefreshServiceControls();RefreshPowerEnabled();RefreshStartupEnabled();RefreshIntegrityEnabled();var historySelection=Get<ListBox>("History").SelectedItem as HistoryRow;Visible("HistoryReport",historySelection!=null&&historySelection.IntegrityCheck);Enabled("HistoryReport",!busy&&historySelection!=null&&historySelection.IntegrityCheck);if(applicationList!=null)ApplicationSelection();RefreshPlanEnabled();foreach(var button in collectionPlanButtons)button.IsEnabled=!busy;Enabled("ProfileImport",!busy);Enabled("ProfileExport",!busy&&preferences.Plan.Count>0);
        ...
        private void ReadHistory() {
            try{var rows=HistoryRows(Path.Combine(Program.Data,"state","applied.dat"),catalogue).Concat(ServiceHistoryRows()).Concat(PowerHistoryRows()).Concat(IntegrityHistoryRows()).Concat(StartupHistoryRows()).OrderByDescending(r=>r.TimeUtc).ToArray();Get<ListBox>("History").ItemsSource=rows;Text("HistoryStatus",rows.Length==0?"Изменений пока нет. После выполнения действия здесь появится запись.":"Запусков: "+rows.Length+". Сначала откатывайте самые новые изменения.");}
        ...
        private async Task Smoke() {
            ...
            await HealthSmoke();await WorkspaceSmoke();await MonitorSmoke();await ApplicationSmoke();await NetworkSmoke();await ServiceManagementSmoke();await ServiceWatchingSmoke();await PowerSmoke();await PlanPersistenceSmoke();await IntegritySmoke();await StartupSmoke();await WindowsRepairSmoke();ShowPage(0);Window.Width=Window.MinWidth;Window.Height=Window.MinHeight;ExpandOutput(true);Window.UpdateLayout();await Task.Delay(100);Capture("portable-ui-compact.png");Assert(IsVisibleInWindow("Apply")&&IsVisibleInWindow("Star")&&IsVisibleInWindow("ActionTitle")&&IsVisibleInWindow("Metadata")&&IsVisibleInWindow("Verify"),"Compact clipping: Apply="+IsVisibleInWindow("Apply")+" Star="+IsVisibleInWindow("Star")+" Title="+IsVisibleInWindow("ActionTitle")+" Metadata="+IsVisibleInWindow("Metadata")+" Verify="+IsVisibleInWindow("Verify"));Capture("portable-ui-compact.png");
