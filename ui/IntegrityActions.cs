...
namespace Wintools {
    ...
    internal static class IntegrityActions {
        internal static string Title(string action){if(action=="dism-scan")return "Проверка компонентов Windows";if(action=="dism-status")return "Статус прошлой проверки компонентов";if(action=="sfc-verify")return "Проверка системных файлов";throw new ArgumentException("Неизвестная проверка Windows.");}
        ...
        internal static async Task<EngineResult> Run(string action,Action<string> progress,Action<Action> cancelReady){
            ...
            using(var cancel=new EventWaitHandle(false,EventResetMode.ManualReset,EventName(id),out created)){
                if(!created)throw new IOException("Не удалось создать независимую проверку.");cancelReady(action=="sfc-verify"?(Action)null:()=>cancel.Set());
        ...
        internal static int Worker(string[] args){
            ...
            using(var cancel=EventWaitHandle.OpenExisting(EventName(id)))using(var log=new StreamWriter(new FileStream(LogPath(id),FileMode.CreateNew,FileAccess.Write,FileShare.Read),new UTF8Encoding(false))){
                string lockPath=Path.Combine(Program.Data,"state","run.lock");Program.SafeDirectory(Path.GetDirectoryName(lockPath));FileStream gate=null;IntegrityRecord record=null;
                try{gate=new FileStream(lockPath,FileMode.CreateNew,FileAccess.Write,FileShare.None);record=new IntegrityRecord{Schema="wintools/integrity/1",Id=id,Action=action,TimeUtc=DateTime.UtcNow.ToString("o"),State="pending",Summary="Проверка начата. Итог ещё не получен."};Save(record);log.WriteLine(Title(action));log.Flush();var sync=new object();Action<string> append=value=>{lock(sync){log.WriteLine(value);log.Flush();}};
                    if(cancel.WaitOne(0))throw new OperationCanceledException("Проверка отменена до начала сканирования.");var result=action=="sfc-verify"?WindowsIntegrity.CheckFiles(append,false):WindowsIntegrity.CheckComponents(action=="dism-scan",cancel,append);record.State=result.State;record.Summary=result.Summary;Save(record);append(result.Summary);return record.State=="failed"?4:0;
