...
namespace Wintools {
    internal sealed partial class MainWindow {
        ...
        private void InitializeStoreActions(StackPanel footer){
            ...
            applicationLaunchEntry=new ComboBox{Width=240,DisplayMemberPath="Name",Margin=new Thickness(0,0,10,0)};System.Windows.Automation.AutomationProperties.SetName(applicationLaunchEntry,"Приложение пакета для запуска");launchRow.Children.Add(applicationLaunchEntry);
            applicationLaunch=new Button{Content="Запустить",IsEnabled=false};launchRow.Children.Add(applicationLaunch);applicationLaunch.Click+=async(s,e)=>await LaunchStoreApplication();
            ...
            applicationStoreRemove=new MenuItem{Header="Удалить для текущего пользователя…"};menu.Items.Add(applicationStoreRemove);applicationStoreRemove.Click+=async(s,e)=>await ChangeStoreApplication("remove");
        ...
        private void StoreSelection(InstalledApplication row){
            ...
            applicationStoreMore.Visibility=package==null?Visibility.Collapsed:Visibility.Visible;applicationStoreMore.IsEnabled=!busy&&!readingApplications;
            applicationFolder.Visibility=applicationRemove.Visibility=package==null?Visibility.Visible:Visibility.Collapsed;applicationStoreRemove.IsEnabled=!busy&&!readingApplications&&package!=null&&!package.Protected;applicationStoreFolder.IsEnabled=applicationFolder.IsEnabled;
