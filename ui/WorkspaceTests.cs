...
namespace Wintools {
    internal sealed partial class MainWindow {
        private async Task WorkspaceSmoke(){
            ...
            var previous=preferences.Plan.ToList();preferences.Plan.Clear();var choice0=collectionChoices[0];AddCollectionToPlan(choice0);Assert(preferences.Plan.Count==20,"Collection bulk addition failed: count="+preferences.Plan.Count+" busy="+busy+" status="+Get<TextBlock>("Status").Text);AddCollectionToPlan(choice0);Assert(preferences.Plan.Count==20,"Collection bulk addition duplicated actions");var lockPath=Path.Combine(Program.Data,"preferences.json.tmp");using(var locked=new FileStream(lockPath,FileMode.OpenOrCreate,FileAccess.ReadWrite,FileShare.None)){AddCollectionToPlan(collectionChoices[1]);Assert(preferences.Plan.Count==20,"Failed collection save mutated plan");}File.Delete(lockPath);preferences.Plan=previous;preferences.Save();RefreshPlan();
