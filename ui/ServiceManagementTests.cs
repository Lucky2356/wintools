...
namespace Wintools {
    internal static class ServiceManagementTests {
        ...
        internal static void Run(){
            ...
            using(var compiler=new CSharpCodeProvider()){var options=new CompilerParameters(new[]{"System.dll","System.ServiceProcess.dll"},exe){GenerateExecutable=true};var result=compiler.CompileAssemblyFromSource(options,"using System.ServiceProcess; public sealed class Fixture : ServiceBase { public Fixture(string name){ServiceName=name;CanStop=true;CanPauseAndContinue=name.EndsWith(\"Child\");} protected override void OnPause(){} protected override void OnContinue(){} public static void Main(string[] args){ServiceBase.Run(new Fixture(args[0]));} }");Assert(!result.Errors.HasErrors,"Service fixture compilation failed");}
            try{
                ...
                Change(name,"pause",null,4);Assert(ServiceActions.Inspect(name).State=="Running","Unsupported pause changed service state");
                var paused=Change(dependent,"pause",null,0);Assert(ServiceActions.Inspect(dependent).State=="Paused","Service did not pause");
                var disabledPaused=Change(dependent,"disabled",null,0);Assert(ServiceActions.Inspect(dependent).State=="Paused","Changing startup resumed paused service");
                var resumed=Change(dependent,"resume",null,0);Assert(ServiceActions.Inspect(dependent).State=="Running","Service did not resume");Change(dependent,"restore",resumed,0);Assert(ServiceActions.Inspect(dependent).State=="Paused"&&ServiceActions.Inspect(dependent).Mode=="Disabled","Restore lost disabled paused state");Change(dependent,"restore",disabledPaused,0);Assert(ServiceActions.Inspect(dependent).State=="Paused"&&ServiceActions.Inspect(dependent).Mode=="Manual","Mode restore lost paused state");Change(dependent,"restore",paused,0);Assert(ServiceActions.Inspect(dependent).State=="Running","Pause rollback did not resume service");
    ...
    internal sealed partial class MainWindow {
        private async Task ServiceManagementSmoke(){
            ...
            try{
                ...
                fixture.Mode="Manual";fixture.CanPause=true;RefreshServiceControls();Assert(servicePause.IsEnabled&&servicePause.Visibility==Visibility.Visible,"Supported pause is not available");var pauseRequest=ChangeSelectedService("pause");Assert(confirmation!=null,"Pause skipped confirmation");FinishConfirmation(false);await pauseRequest;Assert(calls==0,"Cancelled pause executed");fixture.State="Paused";((ServiceState)serviceList.SelectedItem).State="Paused";RefreshServiceControls();Assert(serviceStart.IsEnabled&&(string)serviceStart.Content=="Продолжить"&&serviceStop.IsEnabled&&servicePause.Visibility==Visibility.Collapsed,"Paused service controls incorrect");Window.UpdateLayout();Capture("portable-ui-service-paused.png");fixture.CanPause=false;RefreshServiceControls();Assert(!serviceStart.IsEnabled,"Unsupported resume enabled");fixture.State="Running";((ServiceState)serviceList.SelectedItem).State="Running";fixture.CanPause=false;
