...
namespace Wintools {
    ...
    internal sealed partial class MainWindow {
        ...
        private async Task ReadApplications(){
            ...
            try{int unavailable=0;var desktop=Task.Run(()=>ApplicationInventory.Read(out unavailable));StoreInventory store=null;storeInventoryError="";try{store=await storeRead();}catch(Exception ex){storeInventoryError="Store / MSIX недоступны: "+ex.Message;}var rows=await desktop;if(closed)return;installedApplications=rows.Concat(store==null?new InstalledApplication[0]:store.Rows.Select(StorePackages.Row)).ToArray();if(store!=null&&store.Errors.Length>0)storeInventoryError=string.Join(" ",store.Errors);inaccessibleApplications=unavailable;applicationsLoaded=true;FilterApplications();}catch(Exception ex){if(!closed)applicationStatus.Text="Не удалось прочитать приложения: "+ex.Message;}finally{readingApplications=false;ApplicationSelection();}
        }
