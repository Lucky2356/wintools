...
namespace Wintools {
    internal sealed class InstalledApplication {
        ...
        internal StorePackage Package;
        public string Kind {get{return Package==null?"Обычная программа":"Store / MSIX";}}
    ...
    internal sealed partial class MainWindow {
        ...
        private void InitializeApplications(){
            ...
            applicationKind=new ComboBox{Width=175,Margin=new Thickness(0,0,10,8),ItemsSource=new[]{"Все приложения","Обычные программы","Store / MSIX"},SelectedIndex=0};toolbar.Children.Add(applicationKind);System.Windows.Automation.AutomationProperties.SetName(applicationKind,"Тип приложения");applicationKind.SelectionChanged+=(s,e)=>FilterApplications();
            ...
            applicationStatus=Paragraph("Обычные программы и Store / MSIX текущего пользователя. Portable-программы без регистрации здесь не показываются.");Grid.SetRow(applicationStatus,1);root.Children.Add(applicationStatus);
        ...
        private async Task ReadApplications(){
            if(readingApplications||busy)return;readingApplications=true;ApplicationSelection();applicationStatus.Text="Читаем обычные программы и Store / MSIX…";
            try{int unavailable=0;var desktop=Task.Run(()=>{var result=ApplicationInventory.Read(out unavailable);desktopLaunches=DesktopLaunch.Read();return result;});StoreInventory store=null;storeInventoryError="";try{store=await storeRead();}catch(Exception ex){storeInventoryError="Store / MSIX недоступны: "+ex.Message;}var rows=await desktop;if(closed)return;installedApplications=rows.Concat(store==null?new InstalledApplication[0]:store.Rows.Select(StorePackages.Row)).ToArray();if(store!=null&&store.Errors.Length>0)storeInventoryError=string.Join(" ",store.Errors);inaccessibleApplications=unavailable;applicationsLoaded=true;FilterApplications();}catch(Exception ex){if(!closed)applicationStatus.Text="Не удалось прочитать приложения: "+ex.Message;}finally{readingApplications=false;ApplicationSelection();}
        }
        private void FilterApplications(){if(applicationList==null)return;var previous=applicationList.SelectedItem as InstalledApplication;var query=applicationSearch.Text.Trim();var rows=installedApplications.Where(r=>(applicationKind==null||applicationKind.SelectedIndex==0||(applicationKind.SelectedIndex==1)==(r.Package==null))&&(r.Name+" "+r.Publisher).IndexOf(query,StringComparison.CurrentCultureIgnoreCase)>=0);applicationList.ItemsSource=(applicationSort.SelectedIndex==1?rows.OrderByDescending(r=>r.SizeKb??-1).ThenBy(r=>r.Name):applicationSort.SelectedIndex==2?rows.OrderBy(r=>r.Publisher).ThenBy(r=>r.Name):rows.OrderBy(r=>r.Name)).ToArray();applicationStatus.Text="Показано: "+applicationList.Items.Count+" из "+installedApplications.Length+". Обычных: "+installedApplications.Count(r=>r.Package==null)+" · Store / MSIX: "+installedApplications.Count(r=>r.Package!=null)+" · Недоступных записей реестра: "+inaccessibleApplications+". "+storeInventoryError;if(previous!=null)applicationList.SelectedItem=applicationList.Items.Cast<InstalledApplication>().FirstOrDefault(r=>(r.Package==null)==(previous.Package==null)&&r.Hive==previous.Hive&&r.View==previous.View&&r.Key==previous.Key);}
        private void ApplicationSelection(){var row=applicationList.SelectedItem as InstalledApplication;applicationRemove.IsEnabled=!busy&&!readingApplications&&row!=null&&row.CanRemove&&(row.Msi||!string.IsNullOrWhiteSpace(row.Command));applicationFolder.IsEnabled=!busy&&!readingApplications&&row!=null&&!string.IsNullOrWhiteSpace(row.Location);applicationDetail.Text=row==null?"Выберите программу. Размер сообщён установщиком и может отличаться от занятого места.":row.Name+" · "+(row.Hive==RegistryHive.CurrentUser?"Для текущего пользователя":"Для компьютера")+"\n"+(string.IsNullOrEmpty(row.Location)?"Папка установки не указана.":row.Location);StoreSelection(row);}
        private async Task RemoveApplication(){
            var selected=applicationList.SelectedItem as InstalledApplication;if(selected==null||busy||readingApplications)return;if(selected.Package!=null){await ChangeStoreApplication("remove");return;}
